...
export function CreateLandingClient() {
...
function StepHeader({ title, subtext }: { title: string; subtext?: string }) {
  return (
    <header>
      <h2 className="text-4xl leading-none text-[#0B32A0] sm:text-5xl">
        {title}
      </h2>
      {subtext ? (
        <p className="mt-3 font-body text-base leading-7 text-[#1C1C1C]/65 sm:text-lg">
          {subtext}
        </p>
      ) : null}
    </header>
...
function PillGroup({
  ...
  return (
    <fieldset className="grid gap-3">
      <legend className="font-body text-sm font-semibold uppercase text-[#0B32A0]">
        {title}
      </legend>
            ...
            <button
              ...
              className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${
                selected
                  ...
                  : "border-[#0B32A0]/20 bg-white text-[#0B32A0] hover:border-[#0B32A0]"
              }`}
            >
              {option}
            </button>
