...
function SectionHeader({
  ...
  return (
    <div className="max-w-3xl">
      ...
      <h2 className="mt-3 text-3xl font-semibold leading-tight text-[var(--og-ink)] md:text-5xl">
        {title}
      </h2>
    </div>
  );
  ...
  return (
    <main className="pb-24 md:pb-0">
      <section className="px-4 pb-8 pt-10 md:px-8 md:pt-16 lg:px-12">
        <div className="mx-auto grid max-w-6xl gap-8 overflow-hidden rounded-[2rem] border border-[var(--og-sand)] bg-[rgba(255,248,241,0.86)] p-6 shadow-[0_24px_80px_rgba(22,17,15,0.08)] backdrop-blur md:grid-cols-[1fr_0.9fr] md:p-8">
          <div className="flex flex-col justify-center">
            <p className="text-sm font-semibold uppercase tracking-[0.28em] text-[var(--og-orange)]">
              Custom Hats
            </p>
            <h1 className="mt-4 text-5xl font-semibold leading-none tracking-[-0.04em] text-[var(--og-ink)] md:text-7xl">
              {hatHero.title}
            </h1>
      ...
      <div className="sticky top-[5.75rem] z-30 border-y border-black/10 bg-[rgba(251,247,241,0.9)] px-4 py-3 backdrop-blur md:px-8 lg:px-12">
        <nav className="mx-auto flex max-w-6xl gap-2 overflow-x-auto">
          {hatAnchorLinks.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="inline-flex min-h-10 shrink-0 items-center rounded-full border border-black/10 px-4 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--og-ink)] transition hover:border-[var(--og-orange)] hover:text-[var(--og-orange)]"
            >
              {item.label}
            </Link>
          ))}
        ...
        <section id="overview" className="mx-auto max-w-6xl">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {hatCallouts.map((callout) => (
              ...
              >
                <h3 className="text-2xl font-semibold text-[var(--og-ink)]">
                  {callout.title}
                </h3>
              ...
              <article
                ...
                className={`rounded-[2rem] border p-6 md:p-8 ${
                  index === 0
                    ...
                    : "border-black/10 bg-[rgba(255,248,241,0.88)] text-[var(--og-ink)]"
                }`}
              >
        ...
        <section id="styles" className="mx-auto max-w-6xl">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {hatStyles.map((style) => (
              <article
                key={style.name}
                className="overflow-hidden rounded-[1.75rem] border border-black/10 bg-[rgba(255,248,241,0.88)]"
              >
                ...
                <h3 className="p-5 text-xl font-semibold text-[var(--og-ink)]">
                  {style.name}
                </h3>
              </article>
            ))}
        ...
        <section id="materials" className="mx-auto max-w-6xl">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {hatMaterials.map((material) => (
              <article
                key={material.name}
                className="rounded-[1.5rem] border border-black/10 bg-[rgba(255,248,241,0.88)] p-5"
              >
                ...
                <h3 className="mt-4 text-xl font-semibold text-[var(--og-ink)]">
                  {material.name}
                </h3>
          ...
          <div className="mt-8 grid gap-3">
            {hatFaqs.map((faq) => (
              <details
                ...
                className="rounded-[1.5rem] border border-black/10 bg-[rgba(255,248,241,0.88)] p-5"
              >
                <summary className="cursor-pointer text-lg font-semibold text-[var(--og-ink)]">
                  {faq.question}
                </summary>
                <p className="mt-3 text-base leading-7 text-[var(--og-muted)]">
                  {faq.answer}
