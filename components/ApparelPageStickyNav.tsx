...
type ApparelPageStickyNavProps = {
  mode: "full-custom";
  links?: Array<{ href: string; label: string }>;
};

export function ApparelPageStickyNav({
  mode,
  links = [],
}: ApparelPageStickyNavProps) {
  const handleAnchorClick = (event: MouseEvent<HTMLAnchorElement>, href: string) => {
    if (!href.startsWith("#")) return;

    const target = document.querySelector<HTMLElement>(href);
    const stickyNav = event.currentTarget.closest<HTMLElement>("[data-apparel-sticky-nav]");
