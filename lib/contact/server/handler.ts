...
function isDatabaseCaptureComplete(status: DeliveryStatus | undefined) {
  return status === "delivered" || status === "duplicate";
}

function validateBuilderPayload(payload: Record<string, string>) {
  if (
    payload.source === "og-crafted-hat-builder" &&
    (!payload.company || !payload.email || !payload.name || !payload.phone || !payload.shippingAddress)
  ) {
    return "Missing required builder checkout fields.";
  }

  return "";
}

async function buildSubmissionFromRequest(request: Request) {
  ...
  if (!payload) {
    return { error: "Invalid request body.", requestMeta, status: 400 as const };
  }

  const builderError = validateBuilderPayload(payload);
  if (builderError) {
    return { error: builderError, requestMeta, status: 400 as const };
  }
  ...
  return { requestMeta, submission };
}

async function captureSubmissionInDatabase(submission: SubmissionRecord) {
  await setDeliveryResult(
    submission,
    "stored",
    "skipped",
    "Local JSON persistence is disabled; J-Core database storage is the source of truth.",
  );
  await runDelivery(submission, "artworkStorage", () => (
    syncUploadsToArtworkStorage(submission.payload, submission.uploads, submission.id, submission.submittedAt)
  ));
  await runDelivery(submission, "jcore", () => deliverViaJCore(submission.payload, submission.meta));
}

async function deliverPostCaptureSideEffects(submission: SubmissionRecord) {
  await runDelivery(submission, "hubspot", () => (
    deliverViaHubSpot(submission.payload, submission.uploads, submission.meta)
  ));
  await runDelivery(submission, "webhook", () => deliverViaWebhook(submission.payload));
  await runDelivery(submission, "internalEmail", () => (
    deliverInternalEmail(submission.payload, submission.uploads, submission.meta)
  ));
  await runDelivery(submission, "clientEmail", () => deliverClientConfirmation(submission.payload));

  if (submission.deliveries.jcore?.status !== "duplicate") {
    await runDelivery(submission, "slack", () => deliverHatBuilderSlackNotification(submission.payload));
    return;
  }
  ...
  await setDeliveryResult(
    ...
    "slack",
    "skipped",
    "Skipped because J-Core marked the submission duplicate.",
  );
}

function buildSuccessfulContactResponse(submission: SubmissionRecord) {
...
function buildContactErrorResponse(error: unknown) {
...
export async function POST(request: Request) {
  try {
    const parsed = await buildSubmissionFromRequest(request);
    ...
    await captureSubmissionInDatabase(submission);

    const jcoreStatus = submission.deliveries.jcore?.status;
    if (!isDatabaseCaptureComplete(jcoreStatus)) {
    ...
    await deliverPostCaptureSideEffects(submission);

    console.log("[Contact Submission]", submission);

    return NextResponse.json(buildSuccessfulContactResponse(submission));
  } catch (error) {
    return buildContactErrorResponse(error);
  }
}
